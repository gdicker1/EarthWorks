...
def fetch_source(fsrc):
    cmd = ['git', 'fetch', fsrc]
    stat,oput = exe_ret(cmd)
    if stat != 0:
        msg = f'- Failed to fetch {fsrc} with current remotes'
        print(msg)
        print(f'cmd={cmd}\ncmdOut={oput}\n')
    return stat,oput
...
def checkout_ref(ref):
    ...
    stat,oput = exe_ret(cmd)
    if stat != 0:
        msg = f"- Failed to checkout '{ref}' with current remotes"
        print(msg)
        print(f'cmd={cmd}\ncmdOut={oput}\n')
    return stat,oput
...
def git_recent_tag():
    ...
    if tag_found:
        ...
        msg = "- Failed to get most recent tag from repo"
        print(msg)
        print(f'cmd={cmd}\ncmdOut={oput}\n')
        oput = ''
    return stat,oput
...
def git_tag_msg(tag):
    cmd = ['git', 'tag', '-l', '-n99', tag]
    stat, oput = exe_ret(cmd)
    if stat != 0 or not oput:
        msg = "- Failed to get msg for tag '{tag}'"
        print(msg)
        oput = []
    else:
        oput = os.linesep.join([s.strip() for s in oput.splitlines() if s])

    return oput


def merge_ref(ref, msgs):
    cmd = ['git', 'merge', '--no-ff', ref]
    for msg in msgs:
        cmd.extend(['-m', msg])

    stat,oput = exe_ret(cmd)
    if stat != 0:
        msg = f"- Merge of ref '{ref}' failed"
        print(msg)
        print(f'cmd={cmd}\ncmdOut={oput}\n')
